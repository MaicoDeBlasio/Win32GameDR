...
void main(int3 groupThreadID : SV_GroupThreadID, int3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    RWTexture2D<float> Input  = ResourceDescriptorHeap[cb0.blurMap1UavID];
    RWTexture2D<float> Output = ResourceDescriptorHeap[cb0.blurMap0UavID];
    Texture2D<float4> NormalDepthInput = ResourceDescriptorHeap[cb0.normalDepthMapSrvID];
    //RWTexture2D<float> Input  = ResourceDescriptorHeap[SrvUAVs::AmbientMap1Uav];
    //RWTexture2D<float> Output = ResourceDescriptorHeap[SrvUAVs::AmbientMap0Uav];
    //Texture2D<float4> NormalDepthInput = ResourceDescriptorHeap[SrvUAVs::NormalDepthSrv];

    //RWTexture2D<float4> Input = ResourceDescriptorHeap[SrvUAVs::AmbientMap1Uav];
    //RWTexture2D<float4> Output = ResourceDescriptorHeap[SrvUAVs::AmbientMap0Uav];
    ...
    {
        // Clamp out of bound samples that occur at image borders.
        int y = min(dispatchThreadID.y + BlurRadius, size.y - 1);
        //int x = min(dispatchThreadID.x + gBlurRadius, gInput.Length.x-1);
        gCache[groupThreadID.y + 2 * BlurRadius] = Input[int2(dispatchThreadID.x, y)];
    }

    // Clamp out of bound samples that occur at image borders.
    gCache[groupThreadID.y + BlurRadius] = Input[min(dispatchThreadID.xy, size.xy - 1)];
    //gCache[groupThreadID.x+gBlurRadius] = gInput[min(dispatchThreadID.xy, gInput.Length.xy-1)];

    // Wait for all threads to finish.
    GroupMemoryBarrierWithGroupSync();

    int2 offset = int2(0, 1); // hardcoded vertical blur offset
    //int2 offset;
    //float2 texOffset;
