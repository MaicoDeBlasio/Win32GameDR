...
{
    Texture2D<float3> DiffuseMap = ResourceDescriptorHeap[SrvUAVs::DoveAlbedoSrv];
    Texture2D<float3> NormalMap  = ResourceDescriptorHeap[SrvUAVs::DoveNormalSrv];

    float3 diffuseColor = DiffuseMap.Sample(AnisoClamp, pin.texCoord);
