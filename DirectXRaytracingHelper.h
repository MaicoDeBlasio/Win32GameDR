...
struct AccelerationStructureBuffers
{
    ...
    Microsoft::WRL::ComPtr<ID3D12Resource> instanceDesc = nullptr; // Only applies to TLAS.
    //UINT64 ResultDataMaxSizeInBytes;
};

// Added these
struct ShaderBindingTableBuffer
{
    Microsoft::WRL::ComPtr<ID3D12Resource> buffer;
    uint32_t strideInBytes = 0; // Only applies to hit group and miss shader tables.
};

struct ShaderBindingTableGroup
{
    ShaderBindingTableBuffer raygeneration;
    ShaderBindingTableBuffer hitgroup;
    ShaderBindingTableBuffer miss;
};
...
class ShaderTable : public GpuUploadBuffer
{
    ...
    ShaderTable() {}
public:
    ShaderTable(ID3D12Device* device, uint32_t numShaderRecords, uint32_t shaderRecordSize, LPCWSTR resourceName = nullptr)
        : m_name(resourceName)
    {
    ...
    {
        std::wstringstream wstr; // requires <sstream> in pch.h
        wstr << L"|--------------------------------------------------------------------\n";
...
// Added this function to create resources compatible with enhanced barriers.
inline void AllocateUAVBufferWithLayout(ID3D12Device10* pDevice, UINT64 bufferSize, ID3D12Resource** ppResource,
    const wchar_t* resourceName = nullptr, bool isAccelStructure = false)
{
    ...
    auto bufferDesc = CD3DX12_RESOURCE_DESC1::Buffer(bufferSize, flags);
    DX::ThrowIfFailed(pDevice->CreateCommittedResource3(
        &heapProperties,
        D3D12_HEAP_FLAG_NONE,
        &bufferDesc,
...
// NB: Modified this function. Must pass a pointer to pointer for second parameter as required by Map().
inline void AllocateUploadBuffer(ID3D12Device* pDevice, UINT64 datasize, ID3D12Resource** ppResource, const wchar_t* resourceName = nullptr)
...
//inline void AllocateUploadBuffer(ID3D12Device* pDevice, void **pData, UINT64 datasize, ID3D12Resource **ppResource, const wchar_t* resourceName = nullptr)
{
    ...
    //DX::ThrowIfFailed((*ppResource)->Map(0, &readRange, pData));
    //void *pMappedData;
...
// Added this function to create resources compatible with enhanced barriers.
inline void AllocateUploadBufferWithLayout(ID3D12Device10* pDevice, UINT64 datasize, ID3D12Resource** ppResource, const wchar_t* resourceName = nullptr)
{
    auto heapProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
    auto bufferDesc = CD3DX12_RESOURCE_DESC1::Buffer(datasize);
    DX::ThrowIfFailed(pDevice->CreateCommittedResource3(
        &heapProperties,
        D3D12_HEAP_FLAG_NONE,
        &bufferDesc,
...
inline void PrintStateObjectDesc(const D3D12_STATE_OBJECT_DESC* desc)
{
    ...
    for (uint32_t i = 0; i < desc->NumSubobjects; i++)
    {
        ...
        case D3D12_STATE_SUBOBJECT_TYPE_LOCAL_ROOT_SIGNATURE:
            ...
            break;
        // std::setfill & std::setw require <iomanip> in pch.h
        case D3D12_STATE_SUBOBJECT_TYPE_NODE_MASK:
            wstr << L"Node Mask: 0x" << std::hex << std::setfill(L'0') << std::setw(8)
                 << *static_cast<const uint32_t*>(desc->pSubobjects[i].pDesc) << std::setw(0) << std::dec << L"\n";
            break;
