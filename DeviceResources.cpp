...
void DeviceResources::CreateDeviceResources()
{
    ...
    // NOTE: Enabling the debug layer after device creation will invalidate the active device.
    {
        ...
        if (SUCCEEDED(debugController0->QueryInterface(IID_PPV_ARGS(&debugController1))))
        {
            ...
            // Synchronized command queue validation is on by default for GBV.
            // debugController1->SetEnableSynchronizedCommandQueueValidation(true);
        }
        else
        {
            OutputDebugStringA("WARNING: Direct3D GPU-Based Validation is not available\n");
        }
    ...
    HRESULT hr = D3D12CreateDevice( m_adapter.Get(), //adapter.Get(),
        m_d3dMinFeatureLevel, IID_PPV_ARGS(&m_d3dDevice));
    ThrowIfFailed(hr);
    ...
    if (SUCCEEDED(m_d3dDevice.As(&d3dInfoQueue)))
    {
        ...
        D3D12_MESSAGE_ID hide[] =
        {
            ...
            D3D12_MESSAGE_ID_CREATERESOURCE_STATE_IGNORED, // Suppress warning added in Oct 22H2 cumulative update.
    ...
    ThrowIfFailed(m_d3dDevice->CreateCommandQueue(&queueDesc, IID_PPV_ARGS(&m_computeCommandQueue)));
    m_computeCommandQueue->SetName(L"DeviceResourcesCompute");
    ...
    // Create command allocators for each back buffer that will be rendered to.
    for (uint32_t n = 0; n < m_backBufferCount; n++)
    {
        ...
        ThrowIfFailed(m_d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_COMPUTE,
    ...
    // Create command lists for recording graphics and compute commands.
    ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocators[0].Get(), nullptr,
        IID_PPV_ARGS(&m_commandList)));
    ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_COMPUTE, m_computeCommandAllocators[0].Get(), nullptr,
    ...
    m_computeCommandList->SetName(L"DeviceResourcesCompute");

    // Create fences for tracking GPU execution progress.
    ThrowIfFailed(m_d3dDevice->CreateFence(m_fenceValues[m_backBufferIndex], D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_fence)));
    ThrowIfFailed(m_d3dDevice->CreateFence(m_fenceValues[m_backBufferIndex], D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_computeFence)));
    m_fenceValues[m_backBufferIndex]++;

    m_fence->SetName(L"DeviceResources");
    m_computeFence->SetName(L"DeviceResourcesCompute");
...
void DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    const uint32_t backBufferWidth =  std::max<uint32_t>(static_cast<uint32_t>(m_outputSize.right - m_outputSize.left), 1);
    const uint32_t backBufferHeight = std::max<uint32_t>(static_cast<uint32_t>(m_outputSize.bottom - m_outputSize.top), 1);
    ...
    if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN)
    {
        ...
        depthOptimizedClearValue.DepthStencil.Depth = 0; // We are using a reverse-z depth buffer.
...
{
    // Schedule a Signal command in the direct GPU queue.
...
// Added these.
void DeviceResources::ResetComputeCommandList()
{
...
void DeviceResources::ExecuteAndWaitForGpuCompute()
{
    ThrowIfFailed(m_computeCommandList->Close());
    m_computeCommandQueue->ExecuteCommandLists(1, CommandListCast(m_computeCommandList.GetAddressOf()));

    const UINT64 fenceValue = m_fenceValues[m_backBufferIndex];
    m_computeCommandQueue->Signal(m_computeFence.Get(), fenceValue);
