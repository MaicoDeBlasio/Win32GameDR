#if 0
;
; Note: shader requires additional functionality:
;       Resource descriptor heap indexing
;
;
; Input signature:
;
; Name                 Index   Mask Register SysValue  Format   Used
; -------------------- ----- ------ -------- -------- ------- ------
; no parameters
;
; Output signature:
;
; Name                 Index   Mask Register SysValue  Format   Used
; -------------------- ----- ------ -------- -------- ------- ------
; no parameters
; shader debug name: Shaders\PDB\ComputeShaderAOBlurHorz.pdb
; shader hash: af64d22c23eb9d03cf91464a31c2d8c7
;
; Pipeline Runtime Information: 
;
; Compute Shader
; NumThreads=(64,1,1)
;
;
; Buffer Definitions:
;
; cbuffer cb0
; {
;
;   struct cb0
;   {
;
;       struct struct.BlurConstants
;       {
;
;           float4 blurWeights[3];                    ; Offset:    0
;           uint blurMap0UavID;                       ; Offset:   48
;           uint blurMap1UavID;                       ; Offset:   52
;           uint normalDepthMapSrvID;                 ; Offset:   56
;       
;       } cb0;                                        ; Offset:    0
;
;   
;   } cb0;                                            ; Offset:    0 Size:    60
;
; }
;
;
; Resource Bindings:
;
; Name                                 Type  Format         Dim      ID      HLSL Bind  Count
; ------------------------------ ---------- ------- ----------- ------- -------------- ------
; cb0                               cbuffer      NA          NA     CB0            cb0     1
;
target datalayout = "e-m:e-p:32:32-i1:32-i8:32-i16:32-i32:32-i64:64-f16:32-f32:32-f64:64-n8:16:32:64"
target triple = "dxil-ms-dx"

%dx.types.Handle = type { i8* }
%dx.types.ResBind = type { i32, i32, i32, i8 }
%dx.types.ResourceProperties = type { i32, i32 }
%dx.types.CBufRet.f32 = type { float, float, float, float }
%dx.types.CBufRet.i32 = type { i32, i32, i32, i32 }
%dx.types.Dimensions = type { i32, i32, i32, i32 }
%dx.types.ResRet.f32 = type { float, float, float, float, i32 }
%cb0 = type { %struct.BlurConstants }
%struct.BlurConstants = type { [3 x <4 x float>], i32, i32, i32 }

@"\01?gCache@@3PAMA" = external addrspace(3) global [74 x float], align 4

define void @main() {
  %1 = call %dx.types.Handle @dx.op.createHandleFromBinding(i32 217, %dx.types.ResBind { i32 0, i32 0, i32 0, i8 2 }, i32 0, i1 false)  ; CreateHandleFromBinding(bind,index,nonUniformIndex)
  %2 = call %dx.types.Handle @dx.op.annotateHandle(i32 216, %dx.types.Handle %1, %dx.types.ResourceProperties { i32 13, i32 60 })  ; AnnotateHandle(res,props)  resource: CBuffer
  %3 = call i32 @dx.op.threadIdInGroup.i32(i32 95, i32 0)  ; ThreadIdInGroup(component)
  %4 = call i32 @dx.op.threadId.i32(i32 93, i32 0)  ; ThreadId(component)
  %5 = call i32 @dx.op.threadId.i32(i32 93, i32 1)  ; ThreadId(component)
  %6 = alloca [12 x float], align 4
  %7 = bitcast [12 x float]* %6 to i8*
  call void @llvm.lifetime.start(i64 48, i8* %7) #0
  %8 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 0
  %9 = call %dx.types.CBufRet.f32 @dx.op.cbufferLoadLegacy.f32(i32 59, %dx.types.Handle %2, i32 0)  ; CBufferLoadLegacy(handle,regIndex)
  %10 = extractvalue %dx.types.CBufRet.f32 %9, 0
  store float %10, float* %8, align 4, !tbaa !9
  %11 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 1
  %12 = extractvalue %dx.types.CBufRet.f32 %9, 1
  store float %12, float* %11, align 4, !tbaa !9
  %13 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 2
  %14 = extractvalue %dx.types.CBufRet.f32 %9, 2
  store float %14, float* %13, align 4, !tbaa !9
  %15 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 3
  %16 = extractvalue %dx.types.CBufRet.f32 %9, 3
  store float %16, float* %15, align 4, !tbaa !9
  %17 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 4
  %18 = call %dx.types.CBufRet.f32 @dx.op.cbufferLoadLegacy.f32(i32 59, %dx.types.Handle %2, i32 1)  ; CBufferLoadLegacy(handle,regIndex)
  %19 = extractvalue %dx.types.CBufRet.f32 %18, 0
  store float %19, float* %17, align 4, !tbaa !9
  %20 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 5
  %21 = extractvalue %dx.types.CBufRet.f32 %18, 1
  store float %21, float* %20, align 4, !tbaa !9
  %22 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 6
  %23 = extractvalue %dx.types.CBufRet.f32 %18, 2
  store float %23, float* %22, align 4, !tbaa !9
  %24 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 7
  %25 = extractvalue %dx.types.CBufRet.f32 %18, 3
  store float %25, float* %24, align 4, !tbaa !9
  %26 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 8
  %27 = call %dx.types.CBufRet.f32 @dx.op.cbufferLoadLegacy.f32(i32 59, %dx.types.Handle %2, i32 2)  ; CBufferLoadLegacy(handle,regIndex)
  %28 = extractvalue %dx.types.CBufRet.f32 %27, 0
  store float %28, float* %26, align 4, !tbaa !9
  %29 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 9
  %30 = extractvalue %dx.types.CBufRet.f32 %27, 1
  store float %30, float* %29, align 4, !tbaa !9
  %31 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 10
  %32 = extractvalue %dx.types.CBufRet.f32 %27, 2
  store float %32, float* %31, align 4, !tbaa !9
  %33 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 11
  %34 = extractvalue %dx.types.CBufRet.f32 %27, 3
  store float %34, float* %33, align 4, !tbaa !9
  %35 = call %dx.types.CBufRet.i32 @dx.op.cbufferLoadLegacy.i32(i32 59, %dx.types.Handle %2, i32 3)  ; CBufferLoadLegacy(handle,regIndex)
  %36 = extractvalue %dx.types.CBufRet.i32 %35, 0
  %37 = call %dx.types.Handle @dx.op.createHandleFromHeap(i32 218, i32 %36, i1 false, i1 false)  ; CreateHandleFromHeap(index,samplerHeap,nonUniformIndex)
  %38 = call %dx.types.Handle @dx.op.annotateHandle(i32 216, %dx.types.Handle %37, %dx.types.ResourceProperties { i32 4098, i32 265 })  ; AnnotateHandle(res,props)  resource: RWTexture2D<F32>
  %39 = extractvalue %dx.types.CBufRet.i32 %35, 1
  %40 = call %dx.types.Handle @dx.op.createHandleFromHeap(i32 218, i32 %39, i1 false, i1 false)  ; CreateHandleFromHeap(index,samplerHeap,nonUniformIndex)
  %41 = call %dx.types.Handle @dx.op.annotateHandle(i32 216, %dx.types.Handle %40, %dx.types.ResourceProperties { i32 4098, i32 265 })  ; AnnotateHandle(res,props)  resource: RWTexture2D<F32>
  %42 = extractvalue %dx.types.CBufRet.i32 %35, 2
  %43 = call %dx.types.Handle @dx.op.createHandleFromHeap(i32 218, i32 %42, i1 false, i1 false)  ; CreateHandleFromHeap(index,samplerHeap,nonUniformIndex)
  %44 = call %dx.types.Handle @dx.op.annotateHandle(i32 216, %dx.types.Handle %43, %dx.types.ResourceProperties { i32 2, i32 1033 })  ; AnnotateHandle(res,props)  resource: Texture2D<4xF32>
  %45 = call %dx.types.Dimensions @dx.op.getDimensions(i32 72, %dx.types.Handle %38, i32 0)  ; GetDimensions(handle,mipLevel)
  %46 = extractvalue %dx.types.Dimensions %45, 0
  %47 = extractvalue %dx.types.Dimensions %45, 1
  %48 = icmp slt i32 %3, 5
  br i1 %48, label %49, label %55

; <label>:49                                      ; preds = %0
  %50 = add nsw i32 %4, -5
  %51 = call i32 @dx.op.binary.i32(i32 37, i32 %50, i32 0)  ; IMax(a,b)
  %52 = call %dx.types.ResRet.f32 @dx.op.textureLoad.f32(i32 66, %dx.types.Handle %38, i32 undef, i32 %51, i32 %5, i32 undef, i32 undef, i32 undef, i32 undef)  ; TextureLoad(srv,mipLevelOrSampleCount,coord0,coord1,coord2,offset0,offset1,offset2)
  %53 = extractvalue %dx.types.ResRet.f32 %52, 0
  %54 = getelementptr [74 x float], [74 x float] addrspace(3)* @"\01?gCache@@3PAMA", i32 0, i32 %3
  store float %53, float addrspace(3)* %54, align 4, !tbaa !9
  br label %55

; <label>:55                                      ; preds = %49, %0
  %56 = icmp sgt i32 %3, 58
  %57 = add i32 %46, -1
  br i1 %56, label %58, label %65

; <label>:58                                      ; preds = %55
  %59 = add nsw i32 %4, 5
  %60 = call i32 @dx.op.binary.i32(i32 40, i32 %59, i32 %57)  ; UMin(a,b)
  %61 = call %dx.types.ResRet.f32 @dx.op.textureLoad.f32(i32 66, %dx.types.Handle %38, i32 undef, i32 %60, i32 %5, i32 undef, i32 undef, i32 undef, i32 undef)  ; TextureLoad(srv,mipLevelOrSampleCount,coord0,coord1,coord2,offset0,offset1,offset2)
  %62 = extractvalue %dx.types.ResRet.f32 %61, 0
  %63 = add nsw i32 %3, 10
  %64 = getelementptr [74 x float], [74 x float] addrspace(3)* @"\01?gCache@@3PAMA", i32 0, i32 %63
  store float %62, float addrspace(3)* %64, align 4, !tbaa !9
  br label %65

; <label>:65                                      ; preds = %58, %55
  %66 = add i32 %47, -1
  %67 = call i32 @dx.op.binary.i32(i32 40, i32 %4, i32 %57)  ; UMin(a,b)
  %68 = call i32 @dx.op.binary.i32(i32 40, i32 %5, i32 %66)  ; UMin(a,b)
  %69 = call %dx.types.ResRet.f32 @dx.op.textureLoad.f32(i32 66, %dx.types.Handle %38, i32 undef, i32 %67, i32 %68, i32 undef, i32 undef, i32 undef, i32 undef)  ; TextureLoad(srv,mipLevelOrSampleCount,coord0,coord1,coord2,offset0,offset1,offset2)
  %70 = extractvalue %dx.types.ResRet.f32 %69, 0
  %71 = add nsw i32 %3, 5
  %72 = getelementptr [74 x float], [74 x float] addrspace(3)* @"\01?gCache@@3PAMA", i32 0, i32 %71
  store float %70, float addrspace(3)* %72, align 4, !tbaa !9
  call void @dx.op.barrier(i32 80, i32 9)  ; Barrier(barrierMode)
  %73 = call %dx.types.ResRet.f32 @dx.op.textureLoad.f32(i32 66, %dx.types.Handle %38, i32 undef, i32 %4, i32 %5, i32 undef, i32 undef, i32 undef, i32 undef)  ; TextureLoad(srv,mipLevelOrSampleCount,coord0,coord1,coord2,offset0,offset1,offset2)
  %74 = extractvalue %dx.types.ResRet.f32 %73, 0
  %75 = fmul fast float %74, %21
  %76 = call %dx.types.ResRet.f32 @dx.op.textureLoad.f32(i32 66, %dx.types.Handle %44, i32 0, i32 %4, i32 %5, i32 undef, i32 undef, i32 undef, i32 undef)  ; TextureLoad(srv,mipLevelOrSampleCount,coord0,coord1,coord2,offset0,offset1,offset2)
  %77 = extractvalue %dx.types.ResRet.f32 %76, 0
  %78 = extractvalue %dx.types.ResRet.f32 %76, 1
  %79 = extractvalue %dx.types.ResRet.f32 %76, 2
  %80 = extractvalue %dx.types.ResRet.f32 %76, 3
  br label %83

; <label>:81                                      ; preds = %111
  %82 = fdiv fast float %112, %113
  call void @dx.op.textureStore.f32(i32 67, %dx.types.Handle %41, i32 %4, i32 %5, i32 undef, float %82, float %82, float %82, float %82, i8 15)  ; TextureStore(srv,coord0,coord1,coord2,value0,value1,value2,value3,mask)
  call void @llvm.lifetime.end(i64 48, i8* %7) #0
  ret void

; <label>:83                                      ; preds = %116, %65
  %84 = phi float [ %75, %65 ], [ %117, %116 ]
  %85 = phi float [ %21, %65 ], [ %118, %116 ]
  %86 = phi i32 [ -5, %65 ], [ %119, %116 ]
  %87 = add nsw i32 %71, %86
  %88 = icmp eq i32 %86, 0
  br i1 %88, label %116, label %89

; <label>:89                                      ; preds = %83
  %90 = add i32 %86, %4
  %91 = call %dx.types.ResRet.f32 @dx.op.textureLoad.f32(i32 66, %dx.types.Handle %44, i32 0, i32 %90, i32 %5, i32 undef, i32 undef, i32 undef, i32 undef)  ; TextureLoad(srv,mipLevelOrSampleCount,coord0,coord1,coord2,offset0,offset1,offset2)
  %92 = extractvalue %dx.types.ResRet.f32 %91, 0
  %93 = extractvalue %dx.types.ResRet.f32 %91, 1
  %94 = extractvalue %dx.types.ResRet.f32 %91, 2
  %95 = call float @dx.op.dot3.f32(i32 55, float %92, float %93, float %94, float %77, float %78, float %79)  ; Dot3(ax,ay,az,bx,by,bz)
  %96 = fcmp fast ult float %95, 0x3FE99999A0000000
  br i1 %96, label %111, label %97

; <label>:97                                      ; preds = %89
  %98 = extractvalue %dx.types.ResRet.f32 %91, 3
  %99 = fsub fast float %98, %80
  %100 = call float @dx.op.unary.f32(i32 6, float %99)  ; FAbs(value)
  %101 = fcmp fast ugt float %100, 0x3FC99999A0000000
  br i1 %101, label %111, label %102

; <label>:102                                     ; preds = %97
  %103 = add nsw i32 %86, 5
  %104 = getelementptr inbounds [12 x float], [12 x float]* %6, i32 0, i32 %103
  %105 = load float, float* %104, align 4, !tbaa !9
  %106 = getelementptr [74 x float], [74 x float] addrspace(3)* @"\01?gCache@@3PAMA", i32 0, i32 %87
  %107 = load float, float addrspace(3)* %106, align 4, !tbaa !9
  %108 = fmul fast float %107, %105
  %109 = fadd fast float %108, %84
  %110 = fadd fast float %105, %85
  br label %111

; <label>:111                                     ; preds = %102, %97, %89
  %112 = phi float [ %109, %102 ], [ %84, %97 ], [ %84, %89 ]
  %113 = phi float [ %110, %102 ], [ %85, %97 ], [ %85, %89 ]
  %114 = add nsw i32 %86, 1
  %115 = icmp slt i32 %114, 6
  br i1 %115, label %116, label %81

; <label>:116                                     ; preds = %111, %83
  %117 = phi float [ %112, %111 ], [ %84, %83 ]
  %118 = phi float [ %113, %111 ], [ %85, %83 ]
  %119 = phi i32 [ %114, %111 ], [ 1, %83 ]
  br label %83
}

; Function Attrs: nounwind
declare void @llvm.lifetime.start(i64, i8* nocapture) #0

; Function Attrs: nounwind
declare void @llvm.lifetime.end(i64, i8* nocapture) #0

; Function Attrs: nounwind readnone
declare i32 @dx.op.threadIdInGroup.i32(i32, i32) #1

; Function Attrs: nounwind readnone
declare i32 @dx.op.threadId.i32(i32, i32) #1

; Function Attrs: nounwind readnone
declare %dx.types.Handle @dx.op.createHandleFromHeap(i32, i32, i1, i1) #1

; Function Attrs: nounwind readonly
declare %dx.types.Dimensions @dx.op.getDimensions(i32, %dx.types.Handle, i32) #2

; Function Attrs: nounwind readnone
declare i32 @dx.op.binary.i32(i32, i32, i32) #1

; Function Attrs: nounwind readonly
declare %dx.types.ResRet.f32 @dx.op.textureLoad.f32(i32, %dx.types.Handle, i32, i32, i32, i32, i32, i32, i32) #2

; Function Attrs: nounwind
declare void @dx.op.textureStore.f32(i32, %dx.types.Handle, i32, i32, i32, float, float, float, float, i8) #0

; Function Attrs: noduplicate nounwind
declare void @dx.op.barrier(i32, i32) #3

; Function Attrs: nounwind readnone
declare float @dx.op.dot3.f32(i32, float, float, float, float, float, float) #1

; Function Attrs: nounwind readnone
declare float @dx.op.unary.f32(i32, float) #1

; Function Attrs: nounwind readonly
declare %dx.types.CBufRet.i32 @dx.op.cbufferLoadLegacy.i32(i32, %dx.types.Handle, i32) #2

; Function Attrs: nounwind readonly
declare %dx.types.CBufRet.f32 @dx.op.cbufferLoadLegacy.f32(i32, %dx.types.Handle, i32) #2

; Function Attrs: nounwind readnone
declare %dx.types.Handle @dx.op.annotateHandle(i32, %dx.types.Handle, %dx.types.ResourceProperties) #1

; Function Attrs: nounwind readnone
declare %dx.types.Handle @dx.op.createHandleFromBinding(i32, %dx.types.ResBind, i32, i1) #1

attributes #0 = { nounwind }
attributes #1 = { nounwind readnone }
attributes #2 = { nounwind readonly }
attributes #3 = { noduplicate nounwind }

!llvm.ident = !{!0}
!dx.version = !{!1}
!dx.valver = !{!1}
!dx.shaderModel = !{!2}
!dx.resources = !{!3}
!dx.entryPoints = !{!6}

!0 = !{!"dxcoob 1.7.2308.7 (69e54e290)"}
!1 = !{i32 1, i32 7}
!2 = !{!"cs", i32 6, i32 7}
!3 = !{null, null, !4, null}
!4 = !{!5}
!5 = !{i32 0, %cb0* undef, !"", i32 0, i32 0, i32 1, i32 60, null}
!6 = !{void ()* @main, !"main", null, !3, !7}
!7 = !{i32 0, i64 1073741824, i32 4, !8}
!8 = !{i32 64, i32 1, i32 1}
!9 = !{!10, !10, i64 0}
!10 = !{!"float", !11, i64 0}
!11 = !{!"omnipotent char", !12, i64 0}
!12 = !{!"Simple C/C++ TBAA"}

#endif

const unsigned char g_ComputeShaderAOBlurHorz[] = {
  
};
